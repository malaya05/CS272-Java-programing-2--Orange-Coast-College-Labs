...
public class Controller {
		...
		try
        {
           ...
           int gameId;
            for(ArrayList<String> values : resultsList)
            {
                ...
                // Loop through all the games to find a match
	...
	public boolean addGameToUsersInventory(VideoGame selectedGame)  {
	    ...
	    String[] val = {String.valueOf(mCurrentUser.getId()), String.valueOf(selectedGame.getId())};
	    ...
	    try
        {
            mUserGamesDB.createRecord(USER_GAMES_FIELD_NAMES, val);
        }
        catch (SQLException e)
        {
            e.printStackTrace();
